...
let work url=
    ...
    let a = [for x in matches -> x.Value[8..x.Value.Length - 2] |> fetchAsync] |> Async.Parallel |> Async.RunSynchronously
    Seq.map (fun (t: string*string) -> t |> fst |> String.length, t |> snd) (Seq.choose id a)

let print answer=
    for i, j in answer do printfn $"{i} --- {j}"
    
print (work "https://github.com/PrettyBoyCosmo/HTTP-List")